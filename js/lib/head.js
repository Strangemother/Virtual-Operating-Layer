...
class Assets {
    ...
    static add(d){
        this.loader().addAliases(d)
    }

    static load(name, func) {

        this.loader().load(name, function(){
            Assets._loaded(name, func)
        }.bind({name}))
    }
    ...
    static document(){
        // pseduo document - replacement for a missing browser doc.
        let document = self['document'] || {
            getElementsByTagName(tagName){
                return [{
                    innerHTML: ''
                    , src: ''
                    , id: `auto_tag_${tagName}`
                    , appendChild(item){
                        //console.log('appendChild', item)
                        importScripts(item.src)
                        item.onreadystatechange()
                    }
                    , append() {
...
let assetLoader = function(parent_document){
// asset loader
    return (function(self, undefined){
