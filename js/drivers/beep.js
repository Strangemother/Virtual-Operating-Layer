...
Beep.prototype = {
    ...
    create: function(freq, duration, filters){
        filters = filters || [];
    ...
    , play: function(freq, duration, filters, callback) {
        let audio = this.create(freq, duration, filters, callback)
        audio.play();
...
let standardBeep = new Beep(SAMPLE_RATE)
let standardAmplify = Beep.utils.amplify(tl)

let weeBeep = function(freq=1000, ms=.2, next) {
    standardBeep.play(freq, ms, [standardAmplify], next)
}

let play = function(keys) {
    var key = keys.shift();
    if(typeof key == 'undefined') return; // song ended
    weeBeep(key[0], key[1], function() {
        play(keys);
    });
}

var beepTest = function(count=50){
    let r = [];
    for(var i=0; i < count;i++){
        r.push([
            Math.round(1000 + ( Math.sin(i * 10) * 100 % (count * 4)))
            , 0.04])
    };
    play(r)
}
