...
def convert_templated_dir(dirpath):
    for filepath, _, _ in os.walk(dirpath):
        convert_templated_file(filepath)


def convert_templated_file(filepath):
    in_src_dir = False
    src_dirs = [x for x in SETTINGS.get('SRC_DIRS', ())]
    for src_path in src_dirs:
        if filepath.startswith(src_path):
            in_src_dir = True
            break

    if in_src_dir is True and build_bios.is_templated(filepath) is True:
        print('convert_tpy', filepath)
        import pdb; pdb.set_trace()  # breakpoint 8547e0f2 //

        build_bios.convert_templated(filepath, kwargs=SETTINGS)
    return in_src_dir
...
def copy_libs():
    ...
    convert_templated_dir(out_dir)


def is_tpy(src_path, ext='tpy'):
    return os.path.splitext(src_path)[1] == '.{}'.format(ext)


def is_templated(src_path):
    #print('read', src_path)
    if is_tpy(src_path, 'py') is False:
        return False
    ...
    try:
        ...
        print('Cannot read file', src_path)
        return False
...
def write_startup_file():
    ...
    elif is_templated(src_path):
        convert_templated(src_path, dest_filepath)
